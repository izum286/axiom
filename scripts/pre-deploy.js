...
heading('1. JSON Syntax');
let claudeCode, marketplace;
try {
  claudeCode = JSON.parse(fs.readFileSync(path.join(pluginDir, 'claude-code.json'), 'utf8'));
  console.log('  ✓ claude-code.json valid');
} catch (e) {
  error('json', `claude-code.json parse error: ${e.message}`);
}
try {
  ...
  error('json', `marketplace.json parse error: ${e.message}`);
}

// ── 2. Character Budget ──

heading('2. Character Budget');
if (claudeCode) {
  let total = 0;
  const oversize = [];
  ...
  if (total > 15000) {
    error('budget', `Total ${total}/15,000 chars — EXCEEDS BUDGET (skills invisible to Claude)`);
  } else if (total > 14000) {
    warn('budget', `Total ${total}/15,000 chars — dangerously close to budget`);
  } else {
    console.log(`  ✓ Budget OK: ${total}/15,000 chars (${15000 - total} headroom)`);
  }
...
// ── 3. Manifest ↔ Filesystem Sync ──

heading('3. Manifest ↔ Filesystem Sync');

// Skills: every manifest skill has a SKILL.md on disk
if (claudeCode) {
  const skillsDir = path.join(pluginDir, 'skills');
  for (const skill of claudeCode.skills || []) {
    const skillPath = path.join(skillsDir, skill.name, 'SKILL.md');
    if (!fs.existsSync(skillPath)) {
      error('manifest-sync', `Manifest skill "${skill.name}" has no SKILL.md at ${skill.name}/SKILL.md`);
    }
  }
  console.log(`  ✓ ${claudeCode.skills.length} manifest skills checked against filesystem`);

  // Commands: every manifest command path resolves
  for (const cmdPath of claudeCode.commands || []) {
    const resolved = path.join(pluginDir, cmdPath);
    if (!fs.existsSync(resolved)) {
      error('manifest-sync', `Manifest command path "${cmdPath}" does not exist`);
    }
  }
  console.log(`  ✓ ${claudeCode.commands.length} manifest commands checked against filesystem`);
}

// ── 4. Skill Integrity ──

heading('4. Skill Integrity');
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    if (fs.existsSync(skillFile)) {
      ...
      if (content.trim().length < 50) {
        error('skill-integrity', `${name}/SKILL.md is effectively empty (${content.trim().length} chars)`);
        emptySkills++;
      }
      ...
      if (!fm) {
        ...
        if (!fm.name) error('skill-integrity', `${name}/SKILL.md missing required frontmatter field: name`);
        if (!fm.description) error('skill-integrity', `${name}/SKILL.md missing required frontmatter field: description`);
        if (!fm.license) warn('skill-integrity', `${name}/SKILL.md missing license field`);

        // Name mismatch
        if (fm.name && fm.name !== name) {
          warn('skill-integrity', `${name}/SKILL.md frontmatter name "${fm.name}" doesn't match directory "${name}"`);
        }
      }

      // Duplicate detection
      if (allSkillNames.has(name)) {
        duplicateSkills.push(name);
        error('skill-integrity', `Duplicate skill name: "${name}"`);
      }
      allSkillNames.add(name);
    }

    // Recurse for nested skills (e.g., axiom-ios-ml/coreml/)
    checkSkillsIn(fullPath);
  }
}

checkSkillsIn(path.join(pluginDir, 'skills'));
console.log(`  ✓ ${skillFilesChecked} skill files checked (frontmatter, duplicates, emptiness)`);

// ── 5. Agent Integrity ──

heading('5. Agent Integrity');
...
if (fs.existsSync(agentsDir)) {
  for (const file of fs.readdirSync(agentsDir)) {
    ...
    // Frontmatter check (agents use 'name' or infer from filename)
    const fm = parseFrontmatter(content);
    if (!fm) {
      error('agent-integrity', `${file} has no YAML frontmatter`);
    } else {
      if (!fm.description && !fm.name) {
        error('agent-integrity', `${file} missing both name and description in frontmatter`);
      }
    }
...
if (allSame && versionValues.length > 0) {
  console.log(`  ✓ All ${versionValues.length} files report version ${versionValues[0]}`);
} else {
...
if (fs.existsSync(metadataPath)) {
  ...
  if (metaSkills !== skillFilesChecked) {
    error('metadata', `metadata.txt says ${metaSkills} skills, filesystem has ${skillFilesChecked}`);
  } else {
    console.log(`  ✓ Skill count matches: ${metaSkills}`);
  }

  if (metaAgents !== agentFilesChecked) {
    error('metadata', `metadata.txt says ${metaAgents} agents, filesystem has ${agentFilesChecked}`);
  } else {
    console.log(`  ✓ Agent count matches: ${metaAgents}`);
  }

  if (metaCommands !== commandFilesChecked) {
    error('metadata', `metadata.txt says ${metaCommands} commands, filesystem has ${commandFilesChecked}`);
  } else {
    console.log(`  ✓ Command count matches: ${metaCommands}`);
  }
}

// ── 9. Router Cross-References ──

heading('9. Router Cross-References');

let crossRefChecked = 0;
let brokenRefs = 0;

// Check all router skills for /skill axiom-* references
...
for (const routerName of routerSkillNames) {
  ...
  for (const ref of refs) {
    ...
    if (!allSkillNames.has(targetName)) {
      error('cross-ref', `Router "${routerName}" references non-existent skill "${targetName}"`);
      brokenRefs++;
    }
  }
}
...
if (brokenRefs === 0) {
  console.log(`  ✓ ${crossRefChecked} cross-references validated across ${routerSkillNames.length} routers`);
}

// ── 10. Hook Script Syntax ──

heading('10. Hook Scripts');
...
try {
  ...
  try {
    ...
    error('hooks', `shellcheck failures:\n${e.stdout?.toString() || e.stderr?.toString()}`);
  }
} catch {
  warn('hooks', 'shellcheck not installed (brew install shellcheck) — skipping hook lint');
}

// ── Phase 1 Summary ──

heading('Phase 1 Summary (Static)');
console.log(`  Skills: ${skillFilesChecked} | Agents: ${agentFilesChecked} | Commands: ${commandFilesChecked}`);
console.log(`  Cross-refs: ${crossRefChecked} | Routers: ${routerSkillNames.length}`);
...
try {
  ...
  error('mcp-tests', `MCP server tests failed${summary ? ':\n    ' + summary.join('\n    ') : ''}`);
  console.log(`\n✗ Phase 2 FAILED. Fix MCP tests before deploying.`);
  process.exit(1);
}

// 12. MCP Bundle Build
heading('12. MCP Bundle Build');
try {
  ...
  // Verify bundle.json exists and is non-empty
  ...
  error('mcp-bundle', `MCP bundle build failed: ${e.stderr?.toString()?.slice(0, 200) || 'unknown error'}`);
  console.log(`\n✗ Phase 2 FAILED. Fix MCP bundle before deploying.`);
  process.exit(1);
}

// 13. VitePress Build
heading('13. VitePress Build');
try {
  ...
  const output = e.stdout?.toString() || e.stderr?.toString() || '';
  const deadLinks = output.match(/dead link.*|404.*|DEAD_LINKS.*/gmi);
  error('vitepress', `VitePress build failed${deadLinks ? ':\n    ' + deadLinks.slice(0, 5).join('\n    ') : ''}`);
...
console.log(`  Phase 1: ✓ Static validation (${skillFilesChecked} skills, ${agentFilesChecked} agents, ${commandFilesChecked} commands)`);
